...
Describe 'Checkpoint summary identity integrity' {
    ...
    It 'summarizes a valid checkpoint using canonical path identity' {
        ...
        $checkpoint = [pscustomobject]@{
            ...
            plan = $null
            batches = @(
                [pscustomobject]@{ batchId = '0001'; status = 'Succeeded'; attempts = 1; itemCount = 3; successCount = 3; failedCount = 0; artifactPath = $null; error = $null; updatedUtc = '2026-09-08T00:00:00.0000000Z' },
                [pscustomobject]@{ batchId = '0002'; status = 'Failed'; attempts = 1; itemCount = 2; successCount = 1; failedCount = 1; artifactPath = $null; error = 'failure'; updatedUtc = '2026-09-08T00:00:00.0000000Z' }
            )
        }
