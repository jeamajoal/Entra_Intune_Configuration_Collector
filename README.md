...
- Intune configuration reads used by `intune-core`, including compliance policies, assignment filters, Settings Catalog/configuration policies, classic device configurations, and their assignments, require Microsoft Graph application permission `DeviceManagementConfiguration.Read.All` and an active Intune tenant license. Compliance-policy and classic device-configuration inventory/detail use v1.0. Assignment-filter configuration, Settings Catalog/configuration-policy surfaces, and assignment reads that preserve filter include/exclude IDs/types use beta with truthful beta provenance.
...
- intune-core:
	...
	- configurationPolicies from /beta/deviceManagement/configurationPolicies, admitting only `templateReference.templateFamily` `none` or `deviceConfigurationPolicies`; endpoint-security, baseline, enrollment, script-template, recovery, Company Portal, app quiet-time, unknown/future families are deliberately excluded from this family
...
- During Stage2 resume, a prior successful Graph or on-prem batch is reused only when its existing canonical snapshot declares supported schema version `1.0`, matches current run/stage/section/family/batch identity, and agrees with current planned/checkpoint/snapshot item cardinality; an invalid prior success is reprocessed through the normal Stage2 write/checkpoint path. The paged `configurationPolicySettings` family follows the same rule because each source policy emits one wrapper regardless of how many settings pages are traversed.
...
- Assignment metadata: mobileAppAssignments, deviceManagementScriptAssignments, deviceCompliancePolicyAssignments, configurationPolicyAssignments, deviceConfigurationAssignments, servicePrincipalAppRoleAssignedTo
...
The `intune-core` catalog contract is also additive without a schema-version bump. Stage2 `deviceCompliancePolicies` and `assignmentFilters` depend on their same-named Stage1 inventories; Stage3 `deviceCompliancePolicyAssignments` depends on Stage1 `deviceCompliancePolicies`. Modern Stage2 `configurationPolicies` and `configurationPolicySettings` depend on Stage1 `configurationPolicies`; classic Stage2 `deviceConfigurations` depends on the same-named Stage1 inventory. Stage3 `configurationPolicyAssignments` depends on Stage1 `configurationPolicies`, and `deviceConfigurationAssignments` depends on Stage1 `deviceConfigurations`. Assignment relationship sources remain distinct (`intune.device-compliance-policy`, `intune.configuration-policy`, `intune.device-configuration`) while targets use `entra.group`, `entra.directory-object`, `intune.assignment-filter`, and conservative fallback `intune.assignment-target` as applicable. The catalog does not invent an execution-input dependency on `assignmentFilters`; filter IDs are stable relationship references and the canonical filter family is collected independently by #176.
...
- Intune compliance-policy configuration, assignment filters, Settings Catalog/non-endpoint-security configuration policies, classic device configurations, and assignment targeting/filter context.
...
- Intune per-device/per-user compliance or configuration status, per-setting status/results, status-overview/report telemetry, or remediation.
- Intune endpoint-security/security-baseline policy ownership and enrollment/onboarding configuration in this slice; those remain separately groomed under #178/#181.
