...
function Assert-CollectorInventoryFirstForStage2 {
    ...
    if (-not (Test-CollectorInventoryArtifacts -RunPath $RunPath -Section $Section -Family $Family)) {
        $message = 'Stage2 inventory-first enforcement failed. Missing Stage1 artifact for section {0}, family {1}.' -f $Section, $Family
...
function Invoke-CollectorStage2GraphFamily {
    ...
    foreach ($batch in $batches) {
        ...
        $details=@(); $failedCount=0; $errors=@()
        foreach($inventoryItem in $batchItems){
            $objectId=Get-CollectorObjectId -Item $inventoryItem
            if(-not $objectId){$failedCount++;$errors+='Unable to resolve object id from Stage1 inventory item.';$details += [pscustomobject]@{_collectorError='Unable to resolve object id from Stage1 inventory item.'};continue}
            $endpoint=$EndpointTemplate.Replace('{id}',$objectId)
            try{$details += Invoke-CollectorGraphRequest -GraphToken $Context.GraphToken -Endpoint $endpoint -MaxRetries $Context.MaxRetries -BaseBackoffSeconds $Context.BaseBackoffSeconds -MaxBackoffSeconds $Context.MaxBackoffSeconds -ThrottleMilliseconds $Context.ThrottleMilliseconds}
            catch{$failedCount++;$errors += $_.Exception.Message;$details += [pscustomobject]@{id=$objectId;_collectorError=$_.Exception.Message}}
        }

        $isBeta=$EndpointTemplate.StartsWith('/beta'); $apiVersion=if($isBeta){'beta'}else{'v1.0'}; $status=if($failedCount -eq 0){'Succeeded'}else{'Failed'}
        try{
            ...
            Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null
            $result.failedBatches++;$result.itemCount += $details.Count;$result.errors += $_.Exception.Message
        }
    }
...
function Invoke-CollectorStage2OnPremFamily {
    ...
    param(
        [Parameter(Mandatory=$true)][hashtable]$Context,
        [Parameter(Mandatory=$true)][string]$Section,
        [Parameter(Mandatory=$true)][ValidateSet('domains','organizationalUnits','groups','gpos')][string]$Family
    )

    Assert-CollectorInventoryFirstForStage2 -RunPath $Context.RunPath -Section $Section -Family $Family
    $stageName='stage2';$provenanceProfile=Get-CollectorOnPremProvenanceProfile -Phase 'Details' -Family $Family
    $requestContext=@{cmdletFamily=$Family;cmdletNames=@($provenanceProfile.CmdletNames);inventoryStage='stage1';domainContextFromInventory=$true}
    $checkpoint=Get-CollectorCheckpoint -RunPath $Context.RunPath -RunId $Context.RunId -Stage $stageName -Section $Section -Family $Family
    $result=New-CollectorFamilyResult -Stage $stageName -Section $Section -Family $Family
    $inventoryItems=@(Get-CollectorSnapshotItems -RunPath $Context.RunPath -Stage 'stage1' -Section $Section -Family $Family)
    $batches=Split-CollectorItems -Items $inventoryItems -BatchSize $Context.BatchSize
    ...
    foreach($batch in $batches){
        $batchNumber++;$batchId='{0:D4}' -f $batchNumber
        $decision=Get-CollectorBatchExecutionDecision -Checkpoint $checkpoint -BatchId $batchId -Resume:$Context.Resume -ReprocessFailedOnly:$Context.ReprocessFailedOnly
        if($decision.MarkMissing){
            $existingBatch=Get-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId;$attempts=if($existingBatch){[int]$existingBatch.attempts}else{0};$itemCount=if($existingBatch){[int]$existingBatch.itemCount}else{0};$existingArtifactPath=if($existingBatch){$existingBatch.artifactPath}else{$null}
        ...
        $details=@();$failedCount=0;$errors=@()
        foreach($inventoryItem in $batchItems){try{$details += Invoke-CollectorOnPremDetailFamily -Family $Family -InventoryItem $inventoryItem}catch{$failedCount++;$errors += $_.Exception.Message;$details += [pscustomobject]@{_collectorError=$_.Exception.Message}}}
        ...
        try{
            ...
            $artifact=Write-CollectorSnapshotArtifact -RunPath $Context.RunPath -Stage $stageName -Section $Section -Family $Family -BatchNumber $batchNumber -Snapshot $snapshot;$successCount=[Math]::Max(0,$details.Count-$failedCount)
            $checkpoint=Set-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId -Status $status -Attempts $attempts -ItemCount $details.Count -SuccessCount $successCount -FailedCount $failedCount -ArtifactPath $artifact.artifactPath -Error (($errors -join '; ').Trim());Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null
            if($status -eq 'Succeeded'){$result.succeededBatches++}else{$result.failedBatches++;$result.errors += $errors};$result.itemCount += $details.Count
        }catch{$checkpoint=Set-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId -Status 'Failed' -Attempts $attempts -ItemCount $details.Count -SuccessCount 0 -FailedCount ([Math]::Max(1,$failedCount)) -ArtifactPath $existingArtifactPath -Error $_.Exception.Message;Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null;$result.failedBatches++;$result.itemCount += $details.Count;$result.errors += $_.Exception.Message}
    }
...
function Invoke-CollectorStage2 {
    ...
    foreach($section in $Sections){
        switch($section){
            ...
            default { throw ('Unsupported section for Stage2 detail collection: {0}' -f $section) }
        }
    }
    return $results
}

Export-ModuleMember -Function @('Invoke-CollectorStage2','Assert-CollectorInventoryFirstForStage2')
