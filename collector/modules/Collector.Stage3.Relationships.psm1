...
function Get-CollectorNestedValue {
    ...
    foreach ($propertyName in $DirectProperties) {
        if ($Item.PSObject.Properties.Match($propertyName).Count -gt 0 -and $Item.$propertyName) {
            return [string]$Item.$propertyName
        }
    }

    if ($NestedParent -and $NestedProperty -and $Item.PSObject.Properties.Match($NestedParent).Count -gt 0 -and $Item.$NestedParent) {
        $nestedObject = $Item.$NestedParent
        if ($nestedObject.PSObject.Properties.Match($NestedProperty).Count -gt 0 -and $nestedObject.$NestedProperty) {
            return [string]$nestedObject.$NestedProperty
        }
    }
...
function Test-CollectorStage3ResumeArtifact {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory = $true)]
        [hashtable]$Context,
    ...
    if (
        ...
        $checkpointSuccessCount -ne $checkpointItemCount -or
    ...
    $snapshotItemCount = 0
    if (
        -not [int]::TryParse([string]$snapshot.itemCount, [ref]$snapshotItemCount) -or
        $snapshotItemCount -lt 0 -or
        $snapshotItemCount -ne $checkpointItemCount -or
        @($snapshot.items).Count -ne $snapshotItemCount
    ) {
        return $false
    }
...
function Invoke-CollectorStage3BatchLoop {
    ...
    foreach ($batch in $Batches) {
        ...
        if (
            ...
            -not (Test-CollectorStage3ResumeArtifact -Context $Context -Section $Section -Family $Family -BatchId $batchId -CheckpointBatch $existingBatch)
        ) {
            $recordedItemCount = 0
            if (-not [int]::TryParse([string]$existingBatch.itemCount, [ref]$recordedItemCount) -or $recordedItemCount -lt 0) {
                $recordedItemCount = 0
            }

            $attempts = if ($existingBatch.PSObject.Properties.Match('attempts').Count -gt 0) { [int]$existingBatch.attempts } else { 0 }
            $checkpoint = Set-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId -Status 'Failed' -Attempts $attempts -ItemCount $recordedItemCount -SuccessCount 0 -FailedCount $recordedItemCount -ArtifactPath $existingBatch.artifactPath -Error 'Previously successful Stage3 artifact failed resume validation and will be reprocessed.'
            Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null
        }
        ...
        try {
            ...
            if ($batchOutcome -and $batchOutcome.PSObject.Properties.Match('Items').Count -gt 0) {
                $collectedItems = @($batchOutcome.Items)
            }
            else {
                $collectedItems = @($batchOutcome)
            }

            $hasExplicitFailedCount = $false
            if ($batchOutcome -and $batchOutcome.PSObject.Properties.Match('FailedCount').Count -gt 0) {
                $hasExplicitFailedCount = $true
                $failedCount = [int]$batchOutcome.FailedCount
            }
            else {
                $failedCount = 0
            }

            if ($batchOutcome -and $batchOutcome.PSObject.Properties.Match('Errors').Count -gt 0 -and $batchOutcome.Errors) {
                $errors += @($batchOutcome.Errors)
            }

            if (-not $hasExplicitFailedCount) {
                foreach ($item in $collectedItems) {
                    if ($item -and $item.PSObject.Properties.Match('_collectorError').Count -gt 0 -and $item._collectorError) {
                        $failedCount++
                        $errors += [string]$item._collectorError
                    }
                }
            }
        }
        catch {
            $collectedItems = @([pscustomobject]@{ _collectorError = $_.Exception.Message })
            $failedCount = 1
        ...
        try {
            ...
            $successCount = [Math]::Max(0, $collectedItems.Count - $failedCount)
            $checkpoint = Set-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId -Status $status -Attempts $attempts -ItemCount $collectedItems.Count -SuccessCount $successCount -FailedCount $failedCount -ArtifactPath $artifact.artifactPath -Error (($errors -join '; ').Trim())
            Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null
            ...
            $result.itemCount += $collectedItems.Count
        }
        catch {
            $checkpoint = Set-CollectorCheckpointBatch -Checkpoint $checkpoint -BatchId $batchId -Status 'Failed' -Attempts $attempts -ItemCount $collectedItems.Count -SuccessCount 0 -FailedCount ([Math]::Max(1, $failedCount)) -ArtifactPath $existingArtifactPath -Error $_.Exception.Message
            Save-CollectorCheckpoint -RunPath $Context.RunPath -Checkpoint $checkpoint | Out-Null
    ...
    Invoke-CollectorStage3BatchLoop -Context $Context -Section $Section -Family $Family -Batches $batches -SourceType 'Graph' -SourceName ('Graph {0}' -f $EndpointTemplate) -ApiVersion $apiVersion -IsBeta:$isBeta -RequestContext @{ endpointTemplate = $EndpointTemplate; method = 'GET'; dependencyFamily = $DependencyFamily } -BatchCollector {
        ...
        foreach ($inventoryItem in $batchItems) {
            ...
            if (-not $objectId) {
                ...
                $items += [pscustomobject]@{ _collectorError = 'Unable to resolve object id from Stage1 inventory item.' }
                continue
            }
...
function Invoke-CollectorStage3DelegatedGrantFamily {
    ...
    try {
        $grants = @(Invoke-CollectorGraphCollection -GraphToken $Context.GraphToken -Endpoint '/v1.0/oauth2PermissionGrants' -MaxRetries $Context.MaxRetries -BaseBackoffSeconds $Context.BaseBackoffSeconds -MaxBackoffSeconds $Context.MaxBackoffSeconds -ThrottleMilliseconds $Context.ThrottleMilliseconds)
    }
    catch {
        $seedError = $_.Exception.Message
        $grants = @([pscustomobject]@{ _collectorError = $seedError })
    }
...
function Convert-CollectorPimScheduleToEdge {
    ...
    [pscustomobject]@{
        ...
        scheduleId = Get-CollectorNestedValue -Item $Item -DirectProperties @('id', 'Id')
        principalId = Get-CollectorNestedValue -Item $Item -DirectProperties @('principalId') -NestedParent 'principal' -NestedProperty 'id'
        roleDefinitionId = Get-CollectorNestedValue -Item $Item -DirectProperties @('roleDefinitionId') -NestedParent 'roleDefinition' -NestedProperty 'id'
        directoryScopeId = Get-CollectorNestedValue -Item $Item -DirectProperties @('directoryScopeId') -NestedParent 'directoryScope' -NestedProperty 'id'
        appScopeId = Get-CollectorNestedValue -Item $Item -DirectProperties @('appScopeId') -NestedParent 'appScope' -NestedProperty 'id'
    }
}
...
function Invoke-CollectorStage3PimScheduleEdgeCollection {
    ...
    foreach ($assignment in $assignmentSchedules) {
        $edges += Convert-CollectorPimScheduleToEdge -Item $assignment -ScheduleType 'active'
    }
    foreach ($eligibility in $eligibilitySchedules) {
        $edges += Convert-CollectorPimScheduleToEdge -Item $eligibility -ScheduleType 'eligible'
    }

    $batches = Split-CollectorItems -Items $edges -BatchSize $Context.BatchSize

    Invoke-CollectorStage3BatchLoop -Context $Context -Section $Section -Family 'pimScheduleEdges' -Batches $batches -SourceType 'Derived' -SourceName 'Derived PIM schedule edges from Stage1 inventory' -ApiVersion 'v1.0' -IsBeta:$false -RequestContext @{ dependencyFamilies = @('roleAssignmentScheduleInstances', 'roleEligibilityScheduleInstances'); transform = 'schedule-instance-to-edge' } -BatchCollector {
        param([object[]]$batchItems)
...
function Invoke-CollectorStage3OnPremFamily {
    ...
    Invoke-CollectorStage3BatchLoop -Context $Context -Section $Section -Family $Family -Batches $batches -SourceType 'OnPrem' -SourceName $provenanceProfile.SourceName -ApiVersion 'n/a' -RequestContext $requestContext -BatchCollector {
        ...
        $relationships = @(Invoke-CollectorOnPremRelationshipFamily -Family $Family -InventoryItems $batchItems)
        $errors = @()
        $failedCount = 0

        foreach ($relationship in $relationships) {
            if ($relationship -and $relationship.PSObject.Properties.Match('_collectorError').Count -gt 0 -and $relationship._collectorError) {
                $failedCount++
                $errors += [string]$relationship._collectorError
            }
        }
...
function Invoke-CollectorStage3 {
    ...
    foreach ($section in $Sections) {
        switch ($section) {
            ...
            default {
                throw ('Unsupported section for Stage3 relationship collection: {0}' -f $section)
            }
        }
    }
...
Export-ModuleMember -Function @(
    'Invoke-CollectorStage3',
    'Assert-CollectorInventoryFirstForStage3'
)
